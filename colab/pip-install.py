...
if(len(sys.argv)>=2):
  if(len(sys.argv[1])=="legacy"):
    ...
    output = subprocess.Popen([f"pip install cudf-cu12=={rapids_version}.* cuml-cu12=={rapids_version}.* cugraph-cu12=={rapids_version}.* cuspatial-cu12=={rapids_version}.* cucim-cu12=={rapids_version}.* pylibraft-cu12=={rapids_version}.* raft-dask-cu12=={rapids_version}.* nx-cugraph-cu12=={rapids_version}.* cudf-polars-cu12=={rapids_version}.* cuvs-cu12=={rapids_version}.* aiohttp --extra-index-url=https://pypi.nvidia.com"], shell=True, stderr=subprocess.STDOUT,
      stdout=subprocess.PIPE)
  else:
    rapids_version = COLAB_RAPIDS_VERSION
    print("Installing RAPIDS remaining " + rapids_version + " libraries")
    output = subprocess.Popen([f"pip install cudf-cu12=={rapids_version}.* cuml-cu12=={rapids_version}.* cugraph-cu12=={rapids_version}.* cucim-cu12=={rapids_version}.* pylibraft-cu12=={rapids_version}.* raft-dask-cu12=={rapids_version}.* nx-cugraph-cu12=={rapids_version}.* cudf-polars-cu12=={rapids_version}.* cuvs-cu12=={rapids_version}.* aiohttp --extra-index-url=https://pypi.nvidia.com"], shell=True, stderr=subprocess.STDOUT,
      stdout=subprocess.PIPE)
else:
  rapids_version = COLAB_RAPIDS_VERSION
  print("Installing RAPIDS remaining " + rapids_version + " libraries")
